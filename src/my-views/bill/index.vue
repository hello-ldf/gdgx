<template>
  <div class="app-container">
    <el-tabs v-model="activeName">
      ...
      <!-- 账单列表 tabList -->
      <el-tab-pane label="账单查询" name="list" disabled>
        <!-- 顶部工具 -->
        ...
        <!-- 表格 -->
        <el-table v-loading="tabList.loading" id="table" key="0" :data="tabList.bills" width="100%"
          height="380px" border fit highlight-current-row @row-dblclick="tabList.toBillDetail">
          <el-table-column :label="$t('订单号')" width="180" align="center">
          ...
          <el-table-column :label="$t('负责人')" width="80" align="center">
            <template slot-scope="scope">
          ...
          <el-table-column :label="$t('结余金额')" width="80" align="center">
            <template slot-scope="scope">
        ...
        <pagination v-show="tabList.total>0" :total="tabList.total" :page.sync="tabList.queryParams.page"
          :limit.sync="tabList.queryParams.limit" small background @pagination="tabList.getBills" />

      </el-tab-pane>

      <!-- 账单详细 tabDetail -->
      <el-tab-pane v-loading="tabDetail.loading" label="账单明细" name="detail" disabled>
...
<script>
import { fetchBills, getBillDetail, addBillz } from '@/api/bill'
    ...
    Object.assign(self, {
      ...
      tabList: {
        ...
        getBills: () => { // 获取预订列表
          ...
          fetchBills(self.tabList.queryParams).then(response => {
            self.tabList.bills = response.data.items
        ...
        handleFilter: () => { // 查询预订列表
          ...
          self.tabList.getBills()
        },
        toBillDetail: row => { // 账单详细编辑
          self.activeName = 'detail'
          self.tabDetail.currentOrderNo = row.orderNo
